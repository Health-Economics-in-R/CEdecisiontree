...
```{r}
suppressMessages(
microbenchmark::microbenchmark(dectree_expected_values(dat = dat_long),
                               dectree_expected_recursive(names(tree)[1], tree, dat), times = 100L))
```

For this example the recursive forumulation is much quicker.
Change in memory before and after running the functions.
```{r}
pryr::mem_change(dectree_expected_values(dat = dat_long))
